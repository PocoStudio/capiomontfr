    ...
    try {
      ...
      if (response.status === 429) {
        ...
        if (errorMsg.includes("Trop de requêtes depuis cette IP") || errorMsg.includes("20")) {
          showToast(
            "Votre IP a été bloquée (trop d'envois). Contactez-moi par <a href='mailto:contact@capiomont.fr' class='underline'>email</a>.",
            'ipBlock'
          );
          ...
          showToast(
            "Vous avez atteint la limite de 5 messages par jour. Contactez-moi par <a href='mailto:contact@capiomont.fr' class='underline'>email</a>.",
            'rateLimit'
          );
        } else {
          showToast(
      ...
      if (response.ok) {
        showToast(
          "Message envoyé avec succès ! Je vous contacterai par email dans les plus brefs délais.",
          'success'
        );
        ...
        if (errorMsg && (errorMsg.includes("5 messages par jour") || errorMsg.includes("limite journalière"))) {
          ...
          showToast(
            `Erreur: ${errorMsg}. Contactez-moi par <a href='mailto:contact@capiomont.fr' class='underline'>email</a>.`,
            'error'
          );
        }
      ...
      showToast(
        "Erreur de connexion. Contactez-moi par <a href='mailto:contact@capiomont.fr' class='underline'>email</a>.",
        'error'
      );
            ...
            {selectedOption === 'email' ? (
              ...
              >
                <div className="bento-card-glass p-6 md:p-8 flex flex-col items-center text-center">
                  ...
                  <p className="opacity-80 mb-6">Une question, une suggestion ou une demande de contact ? N'hésitez pas à me solliciter.</p>
                  <a
                ...
                <div className="bento-card-glass p-6 md:p-8">
                  <div className="text-center mb-6">
                    ...
                    <p className="opacity-80 max-w-lg mx-auto">
                      Pour toute information relative à mes projets, demandes professionnelles ou prise de contact, je vous invite à remplir ce formulaire.
                    </p>
                  </div>
                  <FormCard
                    formData={formData}
